...
static	ssize_t my_read(struct file *pFile, char __user *pUser_buff, size_t count, loff_t *pOffset){
    size_t bytes_to_copy;   // Number of bytes that we actually want to copy
    size_t not_copied;      // Number of bytes that copy_to_user() failed to copy
    size_t copied;          // Number of bytes successfully copied

    //Lock the mutex before accessing the shared dev_buffer.
