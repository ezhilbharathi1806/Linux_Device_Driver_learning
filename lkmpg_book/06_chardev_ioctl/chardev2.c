 ...
 * This module
	...
	* Allows:
		open() → open device
		read() → read message from kernel
		write() → write message to kernel
		ioctl() → special control operations
	* Stores data in a kernel buffer (message)
	* Prevents simultaneous access using atomic variable
 */
...
#include "chardev.h"	// Contains Major number and ioctl definitions
